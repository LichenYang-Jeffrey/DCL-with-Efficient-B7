...
class MainModel(nn.Module):
    def __init__(self, backbone, use_dcl=True, numcls=4, use_Asoftmax=False):
        ...
        self.classifier = nn.Linear(2560, self.num_classes, bias=False)

        if self.use_dcl:
            self.classifier_swap = nn.Linear(2560, 2*self.num_classes, bias=False)
            self.Convmask = nn.Conv2d(2560, 1, 1, stride=1, padding=0, bias=True)
