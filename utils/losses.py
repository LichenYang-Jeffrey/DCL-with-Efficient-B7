...
class SupConLoss(nn.Module):
    ...
    def forward(self, inp, labels=None, mask=None):
        ...
        if labels is not None and mask is not None:
            ...
            labels_std = torch.max(labels, 1).indices
            labels_std = labels_std.contiguous().view(-1, 1)
            if labels_std.shape[0] != batch_size:
                raise ValueError('Num of labels does not match num of features')
            mask = torch.eq(labels_std, labels_std.T).float().to(device)
        else:
